...
#for i in {1..30}; do
#R CMD BATCH --no-save --no-restore "--args $i" Speedup1.R &
#done
...
compute_ess <- function(niter, job_id, run_id, partition, permutation, alpha, init_mus, init_sigmas) {
  out_file <- paste0("ESS1_job_", job_id, "_run_", run_id, ".csv")
  ...
  dfcrp_sampler(
    ...
    radius = 75,
    niter = niter,
    output_filename = paste0("ESS1_job_", job_id, "_run_", run_id),
    print_vec = floor(seq(1, niter, length.out = 5)),
  ...
  sigmas <- read.csv(sigma_file, header = F)
...
write.csv(c(total_time, ess), paste0("Speedup1Res_", args, ".csv"), append = T)

quit(save = "no")

# Extract the data for each job
files <- list.files(
  #path = "Speedup Code/Results/W Perm Res",
  pattern = "^Speedup1Res_[0-9]+\\.csv$",
  full.names = TRUE
)
...
time_vec<-numeric()
for (i in 1:30){
  time_vec[i] <- unlist(data_list[[i]][1,2])
}
mean(time_vec)/3600


files <- list.files(
  #path = "Speedup Code/Results/Wout Perm Res",
  pattern = "^Speedup2Res_[0-9]+\\.csv$",
  full.names = TRUE
)
data_list <- lapply(files, read.csv)
...
time_vec2<-numeric()
for (i in c(1:30)){
  time_vec2[i] <- unlist(data_list[[i]][1,2])
}
mean(time_vec2)/3600

mean(time_vec2)/mean(time_vec)
